...
from tests.mock_sensor.affiliate_promoter import MockAffiliatePromoterSensor
from src.sensor.affiliate_promoter import AffiliatePromoterSensor
from src.sensor.interface import TradingSensorInterface, AffiliatePromoterSensorInterface
from src.agent.affiliate_promoter import AffiliatePromoterAgent, AffiliatePromoterPromptGenerator
from src.flows.affiliate_promoter import unassisted_flow as affiliate_promoter_unassisted_flow
from src.constants import FE_DATA_AFFILIATE_PROMOTER_DEFAULTS, FE_DATA_TRADING_DEFAULTS
...
def start_affiliate_promoter_agent(
	agent_type: str,
	session_id: str,
	agent_id: str,
	fe_data: dict,
	genner: Genner,
	rag: RAGInterface,
	sensor: AffiliatePromoterSensorInterface,
	db: DBInterface,
	stream_fn: Callable[[str], None] = lambda x: print(x, flush=True, end=""),
):
	role = fe_data["role"]
	...
	prompt_generator = AffiliatePromoterPromptGenerator(fe_data["prompts"])
	...
	flow_func = partial(
		affiliate_promoter_unassisted_flow,
		agent=agent,
		session_id=session_id,
		role=role,
		time=time_,
	...
	run_cycle(
		...
		session_id,
		agent_id,
		fe_data if agent_type == "affiliate_promoter" else None,
	)


def setup_affiliate_promoter_sensor() -> AffiliatePromoterSensorInterface:
	TWITTER_API_KEY = os.environ["TWITTER_API_KEY"]
	TWITTER_API_KEY_SECRET = os.environ["TWITTER_API_KEY_SECRET"]
	TWITTER_ACCESS_TOKEN = os.environ["TWITTER_ACCESS_TOKEN"]
	TWITTER_ACCESS_TOKEN_SECRET = os.environ["TWITTER_ACCESS_TOKEN_SECRET"]
	...
	sensor = AffiliatePromoterSensor(twitter_client)
	return sensor
...
def starter_prompt():
	...
	questions = [
		...
		inquirer.List(
			...
			choices=["trading", "marketing", "affiliate_promoter"],
			default=["trading"],
		),
		inquirer.List(
			name="rag",
	...
	elif answers["agent_type"] == "affiliate_promoter":
		fe_data = FE_DATA_AFFILIATE_PROMOTER_DEFAULTS.copy()
	...
	for x in range(3):
		if answers["agent_type"] == "marketing":
			...
			start_affiliate_promoter_agent(
				agent_type=answers["agent_type"],
				session_id="default_marketing"
				if answers["agent_type"] == "marketing"
				else "default_trading",
