...
from src.agent.affiliate_promoter import AffiliatePromoterPromptGenerator
from src.agent.trading import TradingPromptGenerator

DATA_FOLDER = "./data"
AFFILIATE_PROMOTER_PROMPT_PATH = "./src/agent/affiliate_promoter.py"
...
affiliate_promoter_default_prompts = AffiliatePromoterPromptGenerator.get_default_prompts()
trading_default_prompts = TradingPromptGenerator.get_default_prompts()

if __name__ == "__main__":
	check_file_committed(TRADING_PROMPT_PATH)
	check_file_committed(AFFILIATE_PROMOTER_PROMPT_PATH)

	trading_prompt_hash = hash(json.dumps(trading_default_prompts, sort_keys=True))
	affiliate_promoter_prompt_hash = hash(json.dumps(affiliate_promoter_default_prompts, sort_keys=True))

	data = {
		"trading": trading_default_prompts,
		"affiliate_promoter": affiliate_promoter_default_prompts,
		"git_info": get_git_info(),
		"trading_prompt_hash": trading_prompt_hash,
		"affiliate_promoter_prompt_hash": affiliate_promoter_prompt_hash,
	}
